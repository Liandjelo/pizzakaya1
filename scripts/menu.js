...
function showDescription(x) {
    ...
    selectBtn.addEventListener('click', () => {
        ...
        cartItems[key] = [0, items.food[x][1], items.food[x][3], quantity.textContent, key, items.food[x][3] * quantity.textContent];
        ...
        document.getElementById('total').textContent = `₱${l}`;
        key++;
        document.getElementById(removeItem.id).addEventListener('click', () => {
            delete cartItems[removeItem.id];
            cart.style.animation = "fade-out 0.3s";
            setTimeout(() => {
                document.getElementById('items').removeChild(cart);
            }, 200);
